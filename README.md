...
> against `composer serve`, set `issuer: 'http://localhost:8003'`.
...
## Local development against the dev auth server

```bash
# 1. Start the auth server (in the auth.stromcom.cz repo)
composer serve   # → http://localhost:8003

# 2. Register a client in /admin/clients with redirect URI http://localhost:9000/callback
#    OR for M2M:
#    composer client:create -- --project=auth --name=my-app --service-account
